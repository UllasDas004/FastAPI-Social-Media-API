...
def delete_post(id : int,db : Session = Depends(get_db),
    ...
    if post.owner_id != current_user_id: # type: ignore
        raise HTTPException(status_code = status.HTTP_403_FORBIDDEN,detail = f"Post with id {id} is not owned by user with id{current_user_id}")
...
def update_post(id : int,post : schemas.PostBase,
    ...
    # conn.commit()
    get_id_query = db.query(models.Post).filter(models.Post.id == id)
    current_post = get_id_query.first()
    if current_post == None:
        raise HTTPException(status_code = status.HTTP_404_NOT_FOUND,detail = f"Post with id {id} not found")
    current_user_id = current_user.id
    if current_post.owner_id != current_user_id: # type: ignore
